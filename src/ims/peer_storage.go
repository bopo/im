...
func (storage *PeerStorage) readPeerIndex() bool {
	...
	for {
		...
		for i := 0; i < n/INDEX_SIZE; i++ {
			...
			_ = binary.Read(buffer, binary.BigEndian, &peer_msgid)
			_ = binary.Read(buffer, binary.BigEndian, &batch_id)
			_ = binary.Read(buffer, binary.BigEndian, &seq_id)
...
func (storage *PeerStorage) savePeerIndex(message_index map[UserID]*UserIndex) {
	...
	for id, value := range (message_index) {
		...
		_ = binary.Write(buffer, binary.BigEndian, value.last_id)
		_ = binary.Write(buffer, binary.BigEndian, value.last_peer_id)
		_ = binary.Write(buffer, binary.BigEndian, value.last_batch_id)
		_ = binary.Write(buffer, binary.BigEndian, value.last_seq_id)
		index += 1
		//batch write to file
		if index%1000 == 0 {
			buf := buffer.Bytes()
