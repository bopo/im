...
class Group(object):
    ...
    def create_group_ext(db, group_id, appid, master, name, is_super, members):
        # db.begin()
        sql = "INSERT INTO `group`(`id`, `appid`, `master`, `name`, `super`) VALUES({}, {}, {}, {}, {})"
        sup = 1 if is_super else 0
        sql = sql.format(group_id, appid, master, name, sup)
        ...
        for m in members:
            sql = "INSERT INTO group_member(group_id, uid) VALUES(%s, %s)" % (group_id, m)
            cursor.execute(sql)
    ...
    def create_group(db, appid, master, name, is_super, members):
        # db.begin()
        sql = "INSERT INTO `group`(appid, master, name, super) VALUES({}, {}, {}, {})"
        sup = 1 if is_super else 0
        sql = sql.format(appid, master, name, sup)
    ...
    def update_group_name(db, group_id, name):
        sql = "UPDATE `group` SET name=%s WHERE id=%s" % (name, group_id)
    ...
    def update_group_super(db, group_id, is_super):
        ...
        logging.debug("update group super:%s", cursor.rowcount)
